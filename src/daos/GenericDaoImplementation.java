...
public class GenericDaoImplementation{
    ...
    public static <T> T getByForeignKey(GenericDao<T> dao, String column, int id) {
        PreparedStatement statement = PreparedStatementFactory.getSelectByForeignKeyStatement(dao.getTableName(), column, id);
        ResultSet resultSet = executeQuery(statement);

        return processResult(dao, resultSet, statement);
    }

    public static <T> T getByColumn(GenericDao<T> dao, String column, String value) {
    ...
    private static <T> T processResult(GenericDao<T> dao, ResultSet resultSet, PreparedStatement statement) {
        T object;
